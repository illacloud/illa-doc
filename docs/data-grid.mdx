...
| `operator`in`items` | String      | Specify the operator. For number type, we support `>, >=, =, <, <=`. For string types, we support contains, equals, startsWith, endsWith, isEmpty, isNot, empty, isAnyOf. |
